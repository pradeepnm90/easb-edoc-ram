// Template: Models (ApiModel.t4) version 3.0

// This code was generated by RAML Server Scaffolder

using System;
using System.Collections.Generic;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using System.ComponentModel.DataAnnotations;

namespace Markel.GlobalRe.Service.Underwriting.API.v1.Models
{
    public partial class EntityLock
    {
        [Required]
		[JsonProperty("entityId")]
		public int entityId { get; set; } // entityId
		[JsonProperty("entityTypeName")]
		public string entityTypeName { get; set; } // entityTypeName
		[JsonProperty("lockedByDisplayName")]
		public string lockedByDisplayName { get; set; } // lockedByDisplayName
		[JsonProperty("lockedTimestamp")]
		public DateTime lockedTimestamp { get; set; } // lockedTimestamp
	}
}